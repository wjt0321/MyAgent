...
| Phase 3 | 生产级基础 | 85% | Swarm、轨迹、成本、安全就绪；Gateway 适配器完整 |
| Phase 4 | 高级特性 | 80% | LSP、TTS、图像、远程桥接、定时任务就绪 |
| Phase 5 | TUI | 95% | 核心框架、对话、工具可视化、权限、配置完成 |
| Phase 6 | Web UI | 85% | 消息编辑、系统提示编辑、会话导入、Token 展示已完成 |
...
### 4.2 已完成的增强
- ✅ 消息编辑和重新发送（点击用户消息旁的编辑按钮）
- ✅ 系统提示编辑（设置面板中直接修改并保存）
- ✅ 会话导入功能（支持 JSON 格式导入）
- ✅ Token 消耗实时展示（header 显示当前会话预估 Token 数）
...
2. 百度文心一言 / 讯飞星火 / 字节豆包 等国内 Provider
