...
- [x] 11.1 `performSearch` 只对第一个匹配项 `scrollIntoView`（避免多个滚动）
- [x] 11.2 搜索栏增加匹配计数显示（`1/12` 格式，无结果时显示"无结果"）
- [x] 11.3 添加"上一个/下一个"导航按钮或快捷键（Enter 下一个，Shift+Enter 上一个）
- [x] 11.4 高亮匹配文本（在 `.content` 中用 `<mark class="search-highlight">` 包裹，当前匹配用 `current` class 增强高亮）

**验证清单**：
- [x] 搜索关键词后滚动到第一个结果
- [x] 匹配计数显示正确（当前/总数）
- [x] 可上下导航匹配项（按钮+快捷键）
- [x] 匹配文本黄色高亮，当前匹配紫色增强高亮
...
**验证清单**：
- [x] 触发工具调用权限请求，确认 banner 风格和交互与 Codex 类似
- [x] 多个 pending 请求可排队展示
- [x] 审批结果以 status chip 展示在消息流中
